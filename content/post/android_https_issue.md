...
There are always https requests in the project, and no third-party SDKs are used. The requests are implemented using HttpUrlConnec tion. It has been safe and sound, and suddenly the tester reported that there was a certificate verification problem. All kinds of viewing materials on the Internet are solved. <!--more-->

The solution is to add a custom certificate to the project or to solve all the problems by trusting all the certificates. However, we did not use a custom certificate. If we trust all the certificates, the security of our own app cannot be guaranteed.
